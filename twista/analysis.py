...
SENTIMENTS = {
    'de': load_german_sentiments()
}

'''
Calculates the mean polarity of a list of part-of-speech derived lemmas (generated by TreeTagger).
...
def polarity(lemmas, lang='de'):
    ...
    values = [sentiments[lemma] for lemma in considered_lemmas]

    return sum(values) / len(considered_lemmas)
...
class TwistaGraph:
    ...
    '''
    Performs a lemmatization of all texts by part-of-speech (POS) tagging using the POS tagger TreeTagger 
    (http://www.cis.uni-muenchen.de/~schmid/tools/TreeTagger/).
    :return graph object
    '''
    def lemmatize(self):
        for src, dest, key, data in tqdm(self.graph.in_edges(data=True, keys=True)):
            lang = data['lang']
            text = data['text']
            if lang in TAGGER:
                tags = treetaggerwrapper.make_tags(TAGGER[lang].tag_text(text))
                lemmas = [t.lemma for t in tags if type(t) is treetaggerwrapper.Tag]
                lemmas = [l for l in lemmas if len(l) >= 3]
                lemmas = [l for l in lemmas if not l.startswith('#')]
                lemmas = [l for l in lemmas if not l.startswith('htt')]
                self.graph[src][dest][key]['lemmas'] = TwistaList(lemmas)
                self.graph[src][dest][key]['polarity'] = polarity(lemmas, lang=lang)
            else:
                self.graph[src][dest][key]['lemmas'] = TwistaList([])
                self.graph[src][dest][key]['polarity'] = None
