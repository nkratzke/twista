...
class Tweet(Concept):
    ...
    def is_quote(self):
        return self.quoted_status() != None and not self.is_reply()

    def is_retweet(self):
        return self.retweeted_status() != None and self.quoted_status() == None
...
class Edge(Concept):
    ...
    def propagated(self):
        return self.src_node().category()

    def src_node(self):
        #node = dict(self.graph.node[self.src])
        #node['id'] = self.src
        return Node(self.src, self.graph)

    def dest_node(self):
        # node = dict(self.graph.node[self.dest])
        # node['id'] = self.dest
...
class Node(Concept):
    ...
    def out_nodes(self, via=lambda e: True):
        ...
        for e in self.out_edges().filter(via):
            ...
            #node = dict(self.graph.node[e.dest])
            #node[id] = e.dest
            if node.type == 'user':
...
class TwistaGraph:
    ...
    def add_to_graph(tweet, graph, last_observation_of):
        ...
        if type(tweet) is Tweet:
            ...
            if tweet.is_retweet():
                src = tweet.retweeted_status().user() #.id()
